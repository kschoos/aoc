...
# Author: Skusku
# Language: Python
# Pretty ugly solution. Happy this is over! Phew...
...
class Node():
    def __init__(self, name_, weight_, children_ = []):
        self.name = name_
        self.children_ = children_
        self.weight = weight_

def get_names(str_):
    return [line.split(' ', 1)[0] for line in str_.split('\n')]

def get_list(str_):
    return str_.split('\n')

def get_data(lst_, name_):
    for line in lst_:
        if line.startswith(name_):
            p = re.compile('(\d+)')
            weight = p.search(line).group()
            children = [child for child in line.replace(' -> ', ', ').split(', ')][1:]
            return (name_, [int(weight), children, []])

def get_data_from_line(line_):
    p = re.compile('(\d+)')
    weight = p.search(line_).group()
    children = [child for child in line_.replace(' -> ', ', ').split(', ')][1:]
    name = line_.split(' ', 1)[0]
    return (name, [int(weight), children, []])


def find_parent(lst_, child_):
    for entry in lst_:
        if child_[0] in entry[1][1]:
            return entry


def solve_part_1(ipt_):
    # sieved_ipt = [entry for entry in ipt_ if '->' in entry]
    for name in get_names(ipt_):
        if ipt_.count(name) == 1:
            return name

def solve_part_2(ipt_):
    # Repeat until finished
        # Check the next entry.
        # If it is a leave.
        # Go to the leave's parent.
        # Add the leave's weight to a weight list in the parent.
        # Delete the leave from the list.
        # Repeat as long until a weight list is found that has 3 entries of which 2 differ.
        # Report the value that would weigh the tower correctly.

    idx = 0
    lst = [get_data_from_line(line) for line in get_list(ipt_)]
    lst_2 = [get_data_from_line(line) for line in get_list(ipt_)]
    dct = {entry[0]: entry[1] for entry in lst}
    dct_unmodified = {entry[0]: entry[1] for entry in lst_2}

    while True:
        if len(lst[idx][1][1]) == 0:
            current = lst[idx]

            if len(current[1][2]) > 0:
                min_value = min(current[1][2])
                max_value = max(current[1][2])
                if min_value == max_value:
                    current[1][0] = current[1][0] + sum(current[1][2])

                elif len([val for val in current[1][2] if val == max_value]) == 1:
                    diff = max_value - min_value
                    return sum(
                        [dct[name][0] - sum(dct[name][2])
                         if dct[name][0] == max_value else 0
                         for name in dct_unmodified[current[0]][1]]) - diff

                elif len([val for val in current[1][2] if val == min_value]) == 1:
                    diff = max_value - min_value
                    return sum(
                        [dct[name][0] - sum(dct[name][2])
                         if dct[name][0] == min_value else 0
                         for name in dct_unmodified[current[0]][1]]) - diff

            parent = find_parent(lst, current)
            parent[1][2].append(current[1][0])
            parent[1][1].remove(current[0])
            current[1][1].append("checked")
...
class TestSolution(unittest.TestCase):
    def test_part_1(self):
        self.assertEqual(solve_part_1(example_input), 'tknk')
        print("Solution for part 1: {}".format(solve_part_1(input)))

    def test_part_2(self):
        self.assertEqual(solve_part_2(example_input), 60)
        print("Solution for part 2: {}".format(solve_part_2(input)))
