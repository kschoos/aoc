...
def format_data(str_ipt_):
    data = [[val #(reg, op, val, con_reg, con_op, con_val)
             for val in line.split(' ') if val != 'if']
            for line in str_ipt_.split('\n')]
    return data

def op(op_code_, val_1, val_2):
    if op_code_ == '>':
        return val_1 > val_2
    if op_code_ == '<':
        return val_1 < val_2
...
def solve_part_1(str_ipt_):
    registers = dict()
    instructions = format_data(str_ipt_)
    print(instructions)
    highest = float('-inf')

    for reg, reg_op, sz, com_reg, com_op, com_sz in instructions:
        if not com_reg in registers:
            registers[com_reg] = 0
        if not reg in registers:
            registers[reg] = 0

        if op(com_op, registers[com_reg], int(com_sz)):
            if reg_op == 'inc':
                registers[reg] += int(sz)
            if reg_op == 'dec':
                registers[reg] -= int(sz)
...
class TestSolution(unittest.TestCase):
    def test_part_1(self):
        self.assertEqual(solve_part_1(example_input)[0], 1)
        print("The Solution to part 1 is: {}".format(solve_part_1(input)[0]))

    def test_part_1(self):
        self.assertEqual(solve_part_1(example_input)[1], 10)
        print("The Solution to part 2 is: {}".format(solve_part_1(input)[1]))
