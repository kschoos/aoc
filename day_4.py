...
input =  \
[lst.split(' ') for lst in '''vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
xzvfbf fopmfxu mvftgr mfupoxf coyhof talcc vpkslo'''.split('\n')]

def solve_part_1(ipt_):
    # Oneliners ftw!
    return len(ipt_) - sum([any([1 if word_2 == word_1 else 0 for idx, word_1 in enumerate(phrase) for word_2 in phrase[idx+1:]]) for phrase in ipt_])

def solve_part_2(ipt_):
    # Because Code-Golf is the way to solve everything.
    return len(ipt_) - sum([any([all([letter in word_1
                                      for letter in word_2]
                                     +
                                     [letter in word_2
                                      for letter in word_1])
                                 for idx, word_1 in enumerate(phrase)
                                 for word_2 in phrase[idx+1:]])
                            for phrase in ipt_])

class TestSolution(unittest.TestCase):
    def test_solution(self):
        self.assertEqual(solve_part_1([['aa', 'bb', 'cc', 'dd', 'ee']]), 1)
        self.assertEqual(solve_part_1([['aa', 'bb', 'cc', 'dd', 'aa']]), 0)
        self.assertEqual(solve_part_1([['aa', 'bb', 'cc', 'dd', 'aaa']]), 1)

        print("The solution for part 1 is: {}".format(solve_part_1(input)))

        self.assertEqual(solve_part_2([['abcde', 'xyz', 'ecdab']]), 0)
        self.assertEqual(solve_part_2([['abcde', 'xyz', 'fghij']]), 1)
        self.assertEqual(solve_part_2([['a', 'ab', 'abc', 'abd', 'abf', 'abj']]), 1)

        print("The solution for part 2 is: {}".format(solve_part_2(input)))
